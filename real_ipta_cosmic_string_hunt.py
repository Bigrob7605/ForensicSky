#!/usr/bin/env python3
"""
🌌 REAL IPTA COSMIC STRING HUNT
Direct analysis of real IPTA DR2 timing files for cosmic string signatures

Target: J1909-3744, J1713+0747, J0437-4715, J1744-1134, J1857+0943
Data: Real IPTA DR2 timing files (.tim) with proper parsing
...
class RealIPTACosmicStringHunt:
    """
    🌌 REAL IPTA COSMIC STRING HUNT
    ...
    def load_real_ipta_data(self):
        """Load real IPTA DR2 timing files for target pulsars"""
        logger.info("📡 Loading real IPTA DR2 timing files for target pulsars...")
    ...
    def load_real_timing_files(self):
        """Load real timing files from IPTA DR2 with proper parsing"""
        timing_data = []
            ...
            for pulsar in self.target_pulsars:
                ...
                for timing_file in timing_files:
                    try:
                        ...
                        # Parse timing data with proper IPTA format
                        for line in lines:
                            line = line.strip()
                            
                            # Skip comments and empty lines
                            if not line or line.startswith('#') or line.startswith('FORMAT') or line.startswith('MODE'):
                                continue
                            
                            # Handle INCLUDE statements
                            if line.startswith('INCLUDE'):
                                include_file = line.split()[1]
                                include_path = timing_file.parent / include_file
                                
                                if include_path.exists():
                                    logger.info(f"       Reading included file: {include_file}")
                                    try:
                                        ...
                                        for include_line in include_lines:
                                            ...
                                            # Skip comments and empty lines
                                            if not include_line or include_line.startswith('#') or include_line.startswith('FORMAT') or include_line.startswith('MODE'):
                                                continue
                                            
                                            
                                            # Parse timing line: filename frequency time residual uncertainty
                                            parts = include_line.split()
                                            if len(parts) >= 4:
                                                try:
                                                    if parts[0] == 'C' and len(parts) >= 5:
                                                        # Format: C frequency time residual uncertainty
                                                        frequency = float(parts[1])
                                                        time = float(parts[2])
                                                        residual = float(parts[3])
                                                        uncertainty = float(parts[4])
                                                    else:
                                                        # Format: filename frequency time residual uncertainty
                                                        frequency = float(parts[1])
                                                        time = float(parts[2])
                                                        residual = float(parts[3])
                                                        uncertainty = float(parts[4])
                                                    ...
                                                    logger.debug(f"Failed to parse include line: {include_line} - {e}")
                                                    continue
                                                except Exception as e:
                                                    logger.debug(f"Unexpected error parsing include line: {include_line} - {e}")
                                                    continue
                                    except Exception as e:
                            ...
                            # Parse direct timing line: C frequency time residual uncertainty
                            parts = line.split()
    ...
    def run_real_ipta_hunt(self):
        """Run the real IPTA cosmic string hunt"""
        logger.info("🌌 STARTING REAL IPTA COSMIC STRING HUNT")
        logger.info("=" * 60)
        ...
        logger.info("🌌 REAL IPTA HUNT COMPLETE!")
        return True
                ...
                if len(pos_frequencies) > 10:
                    ...
                    if len(log_freq) > 5:
                        try:
                            ...
                            valid_mask = np.isfinite(log_freq) & np.isfinite(log_power) & (log_power > 0)
                            if np.sum(valid_mask) > 5:
                                coeffs = np.polyfit(log_freq[valid_mask], log_power[valid_mask], 1)
                                spectral_index = coeffs[0]
                            else:
                                spectral_index = 0  # Default if insufficient valid data
                        except:
                            spectral_index = 0  # Default if calculation fails
                        ...
                        # Use actual measurement uncertainties from timing data
                        uncertainties = np.array([d['uncertainty'] for d in pulsar_timing])
                        mean_uncertainty = np.mean(uncertainties)
                        snr = signal_strength / mean_uncertainty  # Correct Signal-to-noise ratio
                        ...
                        if (kink_dominance and is_non_gaussian) or snr > self.detection_parameters['signal_to_noise_threshold']:
                            superstring_candidates.append({
                                ...
                                'significance': self.calculate_significance(residuals, uncertainties),
    ...
    def compile_results(self):
        ...
        # Save results
        os.makedirs('real_ipta_hunt_results', exist_ok=True)
        
        # Save detailed results
        with open('real_ipta_hunt_results/detailed_results.json', 'w') as f:
            json.dump(self.results, f, indent=2, default=str)
        
        # Save summary
        with open('real_ipta_hunt_results/summary.json', 'w') as f:
            json.dump(summary, f, indent=2, default=str)
        
        # Create visualization
        self.create_real_visualization()
        
        logger.info("✅ Results compiled and saved to real_ipta_hunt_results/")
    ...
    def create_real_visualization(self):
        ...
        try:
            ...
            fig.suptitle('🌌 Real IPTA Cosmic String Hunt Results', fontsize=16, fontweight='bold')
            ...
            plt.tight_layout()
            plt.savefig('real_ipta_hunt_results/real_ipta_hunt_visualization.png', dpi=300, bbox_inches='tight')
            plt.close()
            
            logger.info("✅ Visualization created: real_ipta_hunt_results/real_ipta_hunt_visualization.png")
    ...
    def calculate_significance(self, residuals, uncertainties=None):
        """Calculate significance of signal using proper uncertainties"""
        if len(residuals) < 2:
            return 0
        ...
        # Use actual uncertainties if provided, otherwise use std
        if uncertainties is not None and len(uncertainties) > 0:
            mean_uncertainty = np.mean(uncertainties)
            if mean_uncertainty > 0:
                # Calculate proper signal-to-noise ratio
                snr = abs(mean) / mean_uncertainty
            else:
                snr = abs(mean) / std
        else:
            snr = abs(mean) / std
        
        # Convert to approximate sigma (more conservative)
        return min(snr, 10)  # Cap at 10σ

def main():
    """Main function to run real IPTA cosmic string hunt"""
    print("🌌 REAL IPTA COSMIC STRING HUNT")
    print("=" * 50)
    print("Direct analysis of real IPTA DR2 timing files:")
    print("• Target pulsars: J1909-3744, J1713+0747, J0437-4715, J1744-1134, J1857+0943")
    print("• Real timing files (.tim) from IPTA DR2 with proper parsing")
    print("• High-precision analysis with real astrophysical data")
    print("• Frequency-band analysis, burst detection, correlations")
    print("=" * 50)
    
    hunt = RealIPTACosmicStringHunt()
    success = hunt.run_real_ipta_hunt()
    
    if success:
        print("\n🌌 REAL IPTA HUNT COMPLETE!")
        print("Results saved to: real_ipta_hunt_results/")
        print("Check the visualization: real_ipta_hunt_results/real_ipta_hunt_visualization.png")
    else:
        print("\n❌ REAL IPTA HUNT FAILED!")
