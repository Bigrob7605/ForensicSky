#!/usr/bin/env python3
"""
INJECT COSMIC STRING SKIES
=========================
Fork of forensic script to inject known cosmic string signals
Add one CLI flag: --Gmu 1e-11
Output: new .json with same schema but known string signal layered on top
"""
...
class CosmicStringInjector:
    """Inject cosmic string signals into existing results"""
    
    def __init__(self, input_json: str, Gmu: float = 1e-11):
    ...
    def inject_cosmic_string_signal(self):
        ...
        if 'correlations' in ca:
            ...
            # Cosmic strings produce Hellings-Downs-like correlations
        ...
        self.results['injection_metadata'] = {
            ...
            'description': f'Cosmic string signal with Gμ = {self.Gmu:.2e} injected'
        }
    ...
    def run_injection(self, output_file: str):
        ...
        log.info(f"🚀 STARTING COSMIC STRING INJECTION")
...
def main():
    """Main CLI interface"""
    parser = argparse.ArgumentParser(description='Inject cosmic string signals into PTA results')
    ...
    if args.output is None:
        ...
        args.output = f"{base_name}_injected_Gmu_{args.Gmu:.0e}.json"
    
    # Run injection
    injector = CosmicStringInjector(args.input_json, args.Gmu)
